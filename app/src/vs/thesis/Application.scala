...
object Application {
  ...
  def uniformDistrDistance2D(diagram: IDiagram): Double = {
  ...
  def uniformDistrDistance2D2(diagram: IDiagram): Double = {
    val diagram2 = diagram.asInstanceOf[Diagram2]
    val c = math.Pi / math.sqrt(6)
    val norm = math.sqrt(diagram2.count())
  ...
  def testOptimum() {
    ...
    // for (x <- 0.01 to 3.0 by 0.01) {
    //   val y = (-1 / c) * math.log(1 - math.exp(-c * x))
    //   bw.write("" + math.exp(-c*x) + " " + math.exp((-c)*y))
    //   bw.newLine()
    // }
    ...
    println(uniformDistrDistance2D(d))
    println(uniformDistrDistance2D2(d))
